...
class MessagesController extends GetxController {
  ...
  var chats = <Chat>[].obs;
  late File imageFile;
  late Rx<DocumentSnapshot> lastDocument;
    ...
    _userMessages.listen((QuerySnapshot query) {
      if (query.docs.isNotEmpty) {
        query.docs.forEach((element) {
          messages.addIf(!messages.contains(element),
              Message.fromDocumentSnapshot(element));
        });
  ...
  addMessage(Message _message, String text) {
    ...
    _chatRepository.addMessage(_message, _chat).then((value) {}).then((value) {
      List<User> _users = [];
      _users.addAll(_message.users as Iterable<User>);
